...
class StringFormatterTest {
    ...
    fun testMaskNumbers() {
        ...
        Assert.assertEquals("12 34 56 78 90", maskedValue)
    }
    ...
    fun testMaskAlpha() {
        ...
        Assert.assertEquals("ab cd ef gh ij", maskedValue)
    }

    @Test
    fun testMaskWithCursorPosition() {
        val formatResult = StringFormatter.applyMask(maskAlpha, maskAlphaString, 1)
        Assert.assertEquals("ab cd ef gh ij", formatResult!!.formattedResult)
        Assert.assertEquals(1, formatResult.cursorIndex!!.toLong())
    }
    ...
    fun testUnmaskingMaskedCharacters() {
        ...
        Assert.assertEquals("1234", maskedValue)
    }
    ...
    fun testApplyMaskWithTextWatcherBeforeTextChangedInformationCreditCardNumber() {
        (getAddedTextTests() + getRemovedTextTests() + getReplacedTextCases()).forEach { test ->
            ...
            Assert.assertNotNull(maskResult)
            Assert.assertEquals(test.expectedResult, maskResult!!.formattedResult)
            Assert.assertEquals(test.expectedCaretPosition, maskResult.cursorIndex)
        }
    }
