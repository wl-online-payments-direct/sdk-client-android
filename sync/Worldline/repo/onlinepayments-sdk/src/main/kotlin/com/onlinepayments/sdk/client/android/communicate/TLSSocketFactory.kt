...
internal class TLSSocketFactory(
    ...
    private fun enableTLSOnSocket(socket: Socket): Socket {
        if (socket is SSLSocket) {
            // Only enable protocols that are supported by this device
            val desiredProtocols = arrayOf("TLSv1.2", "TLSv1.3")
            val supportedProtocols = socket.supportedProtocols
            val enabledProtocols = desiredProtocols.filter { it in supportedProtocols }.toTypedArray()
